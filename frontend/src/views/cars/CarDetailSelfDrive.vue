  ...
  <div class="min-h-screen bg-gray-50 pb-20">
    ...
    <div v-else-if="car" class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-10">
      ...
      <nav class="flex mb-8 text-sm font-medium text-gray-400">
        ...
        <span class="mx-2">/</span>
        <span class="text-gray-900">{{ car.brand }} {{ car.model }} - Tự lái</span>
      </nav>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-10">
        ...
        <div class="lg:col-span-2 space-y-8">
          ...
          <div class="bg-white rounded-[2.5rem] shadow-xl shadow-gray-200/50 overflow-hidden border border-gray-100">
            <div class="relative group">
              ...
              <div class="absolute top-6 left-6 flex flex-col gap-3">
                <div class="bg-green-500 text-white px-4 py-2 rounded-2xl shadow-lg flex items-center space-x-2">
                  <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"/></svg>
                  <span class="text-[10px] font-black uppercase tracking-widest">Tự lái</span>
                </div>
            ...
            <div class="p-4 bg-gray-50 flex gap-4 overflow-x-auto no-scrollbar">
              <img
                ...
                :class="activeImage === getCarImageUrl(car, index) ? 'border-green-500 scale-105' : 'border-transparent opacity-60 hover:opacity-100'"
              >
            </div>
          </div>
          ...
          <div class="bg-white rounded-[2.5rem] p-10 border border-gray-100 shadow-sm">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              <!-- Daily Pricing -->
              <div class="bg-green-50/50 p-6 rounded-[2rem] border border-green-100">
                <h3 class="text-sm font-black text-green-600 uppercase mb-4 flex items-center">
                   <span class="w-2 h-2 bg-green-500 rounded-full mr-2"></span> Giá theo ngày
                </h3>
                <div class="space-y-3">
                  ...
                  <div v-if="car.priceWeekend" class="flex justify-between items-center p-3 bg-white rounded-xl shadow-sm">
                    ...
                    <span class="text-sm font-black text-gray-900">{{ formatPrice(car.priceWeekend) }} / Ngày</span>
                  </div>
                </div>
              </div>

              <!-- Hourly Pricing -->
              <div v-if="car.pricePerHour" class="bg-blue-50/50 p-6 rounded-[2rem] border border-blue-100">
                <h3 class="text-sm font-black text-blue-600 uppercase mb-4 flex items-center">
                   <span class="w-2 h-2 bg-blue-500 rounded-full mr-2"></span> Giá theo giờ (4h+)
                </h3>
                <div class="space-y-3">
                  <div class="flex justify-between items-center p-3 bg-white rounded-xl shadow-sm">
                    <span class="text-xs font-bold text-gray-500 uppercase">Giá cơ bản</span>
              ...
              <!-- Monthly Pricing -->
              <div v-if="car.pricePerMonth" class="col-span-full bg-purple-50 p-6 rounded-[2rem] border border-purple-100">
                 <div class="flex items-center justify-between">
                    <div>
                        <h3 class="text-sm font-black text-purple-600 uppercase mb-1">Gói thuê trọn tháng (Ưu đãi)</h3>
                        <p class="text-[10px] text-purple-400 font-bold italic">Ưu tiên phục vụ khách hàng thuê lâu dài</p>
                    </div>
                    <div class="text-xl font-black text-purple-600">{{ formatPrice(car.pricePerMonth) }} / Tháng</div>
                 </div>
              </div>
            </div>
          </div>
          ...
          <!-- Self Drive Benefits -->
          <div class="bg-green-900 rounded-[2.5rem] p-10 text-white shadow-2xl relative overflow-hidden group">
            ...
            <div class="relative z-10">
              <h3 class="text-3xl font-black mb-6 tracking-tight">Lợi ích thuê xe tự lái</h3>
              <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="flex items-start space-x-4">
                  <div class="bg-white/10 p-3 rounded-xl">
                    <svg class="w-6 h-6 text-green-300" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"/></svg>
                  </div>
                  <div>
                    <h4 class="font-bold text-lg mb-2">Tự do di chuyển</h4>
                    <p class="text-green-100 text-sm">Bạn có thể đi bất cứ đâu, bất cứ lúc nào theo lịch trình riêng của mình.</p>
                  </div>
                </div>
                <div class="flex items-start space-x-4">
                  <div class="bg-white/10 p-3 rounded-xl">
                    <svg class="w-6 h-6 text-green-300" fill="currentColor" viewBox="0 0 20 20"><path d="M4 4a2 2 0 00-2 2v4a2 2 0 002 2V6h10a2 2 0 00-2-2H4zm2 6a2 2 0 012-2h8a2 2 0 012 2v4a2 2 0 01-2 2H8a2 2 0 01-2-2v-4zm6 4a2 2 0 100-4 2 2 0 000 4z"/></svg>
                  </div>
                  <div>
                    <h4 class="font-bold text-lg mb-2">Tiết kiệm chi phí</h4>
                    <p class="text-green-100 text-sm">Không mất phí tài xế, phù hợp cho những chuyến đi ngắn hoặc dài hạn.</p>
                  </div>
                </div>
                <div class="flex items-start space-x-4">
                  <div class="bg-white/10 p-3 rounded-xl">
                    <svg class="w-6 h-6 text-green-300" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M2.166 4.999A11.954 11.954 0 0010 1.944 11.954 11.954 0 0017.834 5c.11.65.166 1.32.166 2.001 0 5.225-3.34 9.67-8 11.317C5.34 16.67 2 12.225 2 7c0-.682.057-1.35.166-2.001zm11.541 3.708a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"/></svg>
                  </div>
                  <div>
                    <h4 class="font-bold text-lg mb-2">Bảo hiểm đầy đủ</h4>
                    <p class="text-green-100 text-sm">Được bảo hiểm 100% cho xe và người trong suốt thời gian thuê.</p>
                  </div>
                </div>
                <div class="flex items-start space-x-4">
                  <div class="bg-white/10 p-3 rounded-xl">
                    <svg class="w-6 h-6 text-green-300" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clip-rule="evenodd"/></svg>
                  </div>
                  <div>
                    <h4 class="font-bold text-lg mb-2">Hỗ trợ 24/7</h4>
                    <p class="text-green-100 text-sm">Đội ngũ kỹ thuật luôn sẵn sàng hỗ trợ bạn trong mọi tình huống.</p>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div class="lg:col-span-1">
          <div class="sticky top-24 space-y-6">
            ...
            <div class="glass-dark rounded-[3.5rem] p-10 border border-white/10 shadow-ux-2xl animate-fade-in relative overflow-hidden group/card shadow-primary-900/40">
              <!-- Decorative background detail -->
              <div class="absolute -top-24 -right-24 w-48 h-48 bg-primary-600/10 rounded-full blur-3xl"></div>
              
              <div class="relative z-10">
                <h2 class="text-3xl font-black text-white mb-1 tracking-tighter">{{ car.brand }}</h2>
                <h3 class="text-lg font-bold text-primary-400 mb-8 tracking-tight">{{ car.model }}</h3>
                ...
                <div class="space-y-4 mb-10">
                  <div class="p-6 bg-white/5 rounded-3xl border border-white/5 group hover:border-primary-500/50 transition-all">
                  ...
                  <div v-if="timeMode === 'hourly'" class="p-6 bg-white/5 rounded-3xl border border-white/5 flex items-center justify-between group hover:border-primary-500/50 transition-all">
                  ...
                  <div class="p-6 bg-primary-500/10 rounded-3xl border border-primary-500/20 group hover:border-primary-500/50 transition-all">
                    <label class="block text-[8px] text-primary-400 font-black uppercase tracking-[0.3em] mb-2">Địa điểm nhận xe</label>
                    <div class="font-black text-xs text-white flex items-start">
                      <svg class="w-4 h-4 mr-2 text-primary-500 mt-0.5" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M5.05 4.05a7 7 0 119.9 9.9L10 18.9l-4.95-4.95a7 7 0 010-9.9zM10 11a2 2 0 100-4 2 2 0 000 4z" clip-rule="evenodd"/></svg>
                ...
                <!-- Cost Summary (High Contrast Design) -->
                <div class="pt-10 border-t border-white/10 space-y-4">
                  <div class="flex justify-between items-center text-[10px] group/item">
                    <span class="text-gray-500 font-black uppercase tracking-[0.2em] group-hover/item:text-gray-300 transition-colors">
                      Giá thuê (x{{ timeMode === 'daily' ? rentalDays + ' ngày' : hoursCount + ' giờ' }})
                    </span>
                    <span class="font-black text-white">
                      {{ formatPrice(timeMode === 'daily'
                          ? car.pricePerDay * rentalDays
                          : car.pricePerHour * hoursCount)
                      }}
                    </span>
                  </div>
                  <div class="flex justify-between items-center text-[10px] group/item">
                  ...
                  <div class="pt-8 mt-4 border-t border-white/10">
                    <p class="text-[9px] font-black text-gray-500 uppercase tracking-[0.3em] mb-3 leading-none">Tổng giá trị hợp đồng</p>
                    <span class="text-4xl font-black text-primary-500 tracking-tighter leading-none block drop-shadow-lg">{{ formatPrice(totalCost) }}</span>
                  </div>
                </div>

                <button @click="handleBooking" class="w-full mt-10 py-5 bg-primary-600 hover:bg-primary-500 text-white font-black rounded-3xl shadow-2xl shadow-primary-900/40 uppercase tracking-[0.3em] transition-all active-scale text-[11px] border border-primary-400/20">
                  XÁC NHẬN ĐẶT XE
                </button>
              </div>
            </div>
            ...
            <div class="p-8 bg-gray-900/80 backdrop-blur-md rounded-[2.5rem] border border-white/10 shadow-xl overflow-hidden relative group/ins">
              ...
              <div class="absolute inset-0 bg-gradient-to-r from-green-500/10 to-transparent opacity-0 group-hover/ins:opacity-100 transition-opacity duration-700"></div>
              
              <div class="flex items-start space-x-4 relative z-10">
                <div class="bg-green-500 rounded-2xl p-2.5 shadow-lg shadow-green-900/40">
                  <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z"/></svg>
                </div>
                <div>
                  <h4 class="font-black text-white text-sm uppercase tracking-widest leading-none mb-2">Bảo vệ toàn diện</h4>
                  <p class="text-[10px] text-gray-400 font-medium leading-relaxed">Quý khách được bảo hiểm 100% tài sản và con người trong suốt hành trình thuê xe tự lái.</p>
                </div>
              </div>
            </div>
          </div>
...
const serviceFee = computed(() => {
  ...
  const rentalFee = timeMode.value === 'daily' 
    ? car.value.pricePerDay * rentalDays.value 
    : car.value.pricePerHour * hoursCount.value
...
const fetchCar = async () => {
  try {
    ...
    // isSelfDrive defaults to true in schema; only redirect if explicitly false
    if (car.value.isSelfDrive === false) {
      toast.error('Xe này không hỗ trợ thuê tự lái')
      router.push('/cars')
      return
    }
...
const handleBooking = () => {
  router.push({
    path: `/cars/${car.value._id}/self-drive/booking`,
    query: {
      mode: timeMode.value,
